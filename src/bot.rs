...
impl TradingBot {
    pub fn new(config: Config, exchange: Arc<Mutex<dyn ExchangeClient>>, state: BotState) -> Self {
        Self {
            config,
            exchange,
    ...
    pub async fn run_cycle_with_time(
        &mut self,
    ...
    pub async fn run_cycle_with_options(
        &mut self,
        simulation_time: Option<DateTime<Utc>>,
        fear_greed_override: Option<FearGreedIndex>,
    ) -> anyhow::Result<()> {
        ...
        self.check_buy_opportunities(&fear_greed_index, &price_map, simulation_time)
            .await?;
    ...
    async fn get_current_prices(&self) -> anyhow::Result<Vec<Price>> {
        ...
        exchange
            .get_prices(&[self.config.assets.crypto_symbol.clone()])
            .await
    }
    ...
    async fn check_sell_opportunities(
        &mut self,
    ...
    async fn check_buy_opportunities(
        ...
        let (dip_signal, dip_percent) =
            if let Some(&current_price) = current_prices.get(crypto_symbol) {
                let is_dip = self.state.is_price_dip(
                    crypto_symbol,
                    current_price,
                    self.config.trading.buy_the_dip_percent,
                );
        ...
        if !fear_greed_signal && !dip_signal {
            println!(
                "No buy signals: F&G: {} (threshold: {}), Dip signal: {} ({}% threshold)",
                fear_greed_index.value,
                self.config.trading.fear_greed_threshold,
                dip_signal,
                self.config.trading.buy_the_dip_percent
            );
            return Ok(());
        ...
        if fear_greed_signal {
            println!(
                "Fear & Greed buy signal triggered: {} <= {}",
                fear_greed_index.value, self.config.trading.fear_greed_threshold
            );
        }
        if dip_signal {
            println!(
                "Buy the dip signal triggered: price dropped {:.2}% from recent high (threshold: {}%)",
        ...
        if self.state.active_baskets.len() >= self.config.trading.basket_count as usize {
            println!(
                "All basket slots are occupied ({}/{})",
                self.state.active_baskets.len(),
                self.config.trading.basket_count
            );
            return Ok(());
        }
        ...
        if let Some(&_current_price) = current_prices.get(crypto_symbol) {
            println!(
                "Creating new basket for {crypto_symbol} with investment amount {investment_amount}"
            );
    ...
    fn calculate_investment_percent(&self, fear_greed_value: u32) -> Decimal {
        ...
        let range =
            self.config.trading.max_investment_percent - self.config.trading.min_investment_percent;
    ...
    fn calculate_dip_investment_percent(&self, dip_percent: Decimal) -> Decimal {
        ...
        println!(
            "Dip-based investment: {:.2}% dip -> {:.2}% investment (range: {:.1}%-{:.1}%)",
            dip_percent,
            investment_percent,
        ...
        if !self.state.active_baskets.is_empty() {
            ...
            for (i, basket) in self.state.active_baskets.iter().enumerate() {
                if let Some(&current_price) = current_prices.get(&basket.asset) {
                    ...
                    let target_price = basket.buy_price
                        * (Decimal::from(100) + basket.target_profit_percent)
                        / Decimal::from(100);
                    let current_time = simulation_time.unwrap_or_else(chrono::Utc::now);
                    ...
                    println!(
                        "  Quantity: {:.6} | Target: ${:.2} ({:.1}%)",
                        basket.quantity, target_price, basket.target_profit_percent
                    );
                    println!(
                        "  Current: ${current_price:.2} | Value: ${current_value:.2} | P&L: ${profit:.2} ({profit_percent:.2}%)"
                    );
                    println!("  Invested: ${invested_amount:.2} | Days held: {days_held}");
                } else {
                    println!(
                        "Basket {}: {} (no current price available)",
                        i + 1,
                        basket.asset
                    );
                }
